...
class RestrictedZone:
    def __init__(self, pts):
        self.pts = pts  # points for border of polygon
    ...
    def is_point_in_zone(self, test_pt):
        dist = cv2.pointPolygonTest(self.pts, test_pt, False)
...
out = cv2.VideoWriter('transformed_output.mp4', fourcc, frame_rate, my_transform.size)
...
while success:  # loop through all the frames
    if True:  # counter % 20 == 0: #adjust if it's too slow
        ...
        for (xA, yA, xB, yB) in boxes:
            ...
            #               (0, 255, 0), 2)
            centroid = (int((xA + xB) / 2), int(yB))  # centroid in original
        ...
        # cv2.imshow(str(counter), image_new)  # Transformed Capture
        # cv2.waitKey(200)
